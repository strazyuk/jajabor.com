...
<style>
    ...
    .navbar-custom {
        background: linear-gradient(rgba(0, 0, 0, 0.6),
                rgba(0, 0, 0, 0.6)), url('your-image-path.jpg') center/cover no-repeat;
        color: #ffffff;
        padding: 1.5rem 2rem;
    }
                ...
                <div class="p-4">
                    ...
                    <p class="text-gray-700 mb-4">
                        <strong>Total Price:</strong> <span
                            id="discount-price">${{ $flight->price *= $passengers }}</span>
                    </p>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" id="coupon-code" class="w-full border border-gray-300 rounded-lg p-2"
                            placeholder="Enter coupon code">
                        <button type="button" onclick="applyCoupon()"
                            class="mt-2 bg-blue-500 text-white py-2 px-4 rounded-lg shadow-md hover:bg-blue-600 transition-all duration-300">
                            Apply Coupon
                        </button>
                    ...
                    <form action="{{ route('payment.createCheckoutSession', $flight->id) }}" method="POST">
                        ...
                        <input type="hidden" name="discounted_price" id="discounted-price-input"
                            value="{{ $flight->price * $passengers }}">
                        <input type="hidden" name="coupon_code" id="coupon-code-input" value="">
                        <button type="submit"
                            class="w-full bg-green-500 text-white py-3 rounded-lg shadow-md hover:bg-green-600 transition-all duration-300">
                            Proceed to Payment
                        </button>
                    </form>

                    <a href="{{ route('flights.index') }}"
                        class="inline-block mt-4 text-blue-500 hover:underline transition-all duration-300">
                        Cancel
                    </a>
                </div>
            </div>
