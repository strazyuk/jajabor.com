        ...
        <main class="flex-1 p-6 md:p-10 overflow-y-auto">
            <div class="max-w-7xl mx-auto space-y-10">
                <header class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h1 class="text-3xl font-black text-gray-900 dark:text-white tracking-tight">Special <span
                                class="text-indigo-600">Offers</span></h1>
                        <p class="text-gray-500 dark:text-gray-400 font-medium mt-1">Manage promotional banners and
                            flash deals.</p>
                    </div>
                            ...
                            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                                @forelse($offers as $offer)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition">
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="font-bold text-gray-900 dark:text-white">{{ $offer->title }}</div>
                                            <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                                                {{ $offer->subtitle }}</div>
                                        </td>
                                        <td class="px-6 py-4 font-mono font-bold text-indigo-600">
                                            {{ $offer->discount_code ?? 'None' }}
                                        </td>
                                        <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">
                                            {{ $offer->valid_until ? \Carbon\Carbon::parse($offer->valid_until)->format('M d, Y') : 'Indefinite' }}
                                        </td>
                                        <td class="px-6 py-4">
                                            @if($offer->is_active && (!$offer->valid_until || $offer->valid_until > now()))
                                                <span
                                        ...
                                        <td class="px-6 py-4 text-right space-x-2">
                                            ...
                                            <form action="{{ route('admin.offers.destroy', $offer) }}" method="POST"
                                                class="inline-block" onsubmit="return confirm('Delete this offer?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6" class="px-6 py-8 text-center text-gray-500 font-medium">No offers
                                            found.</td>
                                    </tr>
                                @endforelse
                            </tbody>
