                ...
                <header class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h1 class="text-3xl font-black text-gray-900 dark:text-white tracking-tight">Travel <span
                                class="text-indigo-600">Intelligence</span></h1>
                        <p class="text-gray-500 dark:text-gray-400 font-medium mt-1">Manage news articles, guides, and
                            updates.</p>
                    </div>
                            ...
                            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                                @forelse($newsItems as $news)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition">
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="font-bold text-gray-900 dark:text-white">{{ $news->title }}</div>
                                            <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                                                {{ Str::limit($news->excerpt, 50) }}</div>
                                        </td>
                                        <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">
                                            {{ $news->published_at ? \Carbon\Carbon::parse($news->published_at)->format('M d, Y') : 'Draft' }}
                                        </td>
                                        <td class="px-6 py-4">
                                            @if($news->is_published)
                                                <span
                                                    class="px-3 py-1 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 text-xs font-bold rounded-full">Published</span>
                                            @else
                                                <span
                                                    class="px-3 py-1 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 text-xs font-bold rounded-full">Draft</span>
                                            @endif
                                        </td>
                                        <td class="px-6 py-4 text-right space-x-2">
                                            ...
                                            <form action="{{ route('admin.news.destroy', $news) }}" method="POST"
                                                class="inline-block" onsubmit="return confirm('Delete this story?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="px-6 py-8 text-center text-gray-500 font-medium">No
                                            intelligence stories found.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
