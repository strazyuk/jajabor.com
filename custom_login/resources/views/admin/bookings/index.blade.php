    ...
    <div class="min-h-screen bg-gray-50 dark:bg-gray-900 flex transition-colors duration-300">
        ...
        <main class="flex-1 p-6 md:p-10 overflow-y-auto">
            <div class="max-w-7xl mx-auto space-y-10">
                ...
                <header class="mb-10">
                    <h1 class="text-3xl font-black text-gray-900 dark:text-white tracking-tight">Booking <span
                            class="text-indigo-600">Oversight</span></h1>
                    <p class="text-gray-500 dark:text-gray-400 font-medium mt-1">Monitor all platform transactions and
                        reservations.</p>
                </header>

                <!-- Flight Bookings Bento -->
                <div
                                ...
                                @forelse($flightBookings as $booking)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition">
                                        <td class="px-6 py-4 font-bold text-gray-900 dark:text-white">
                                            #FL-{{ str_pad($booking->id, 5, '0', STR_PAD_LEFT) }}
                                        </td>
                                        <td class="px-6 py-4">
                                        ...
                                        <td class="px-6 py-4 text-sm font-medium text-gray-500">
                                            {{ $booking->created_at->format('M d, Y h:ia') }}
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="5" class="px-6 py-8 text-center text-gray-500 font-medium">No flight
                                            bookings found.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                    ...
                    @if($flightBookings->hasPages())
                        <div class="px-6 py-4 border-t border-gray-100 dark:border-gray-700 border-t">
                            {{ $flightBookings->links() }}
                        </div>
                    @endif
                </div>

                <!-- Hotel Bookings Bento -->
                <div
                    class="bg-white dark:bg-gray-800 rounded-[2.5rem] shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
                    <div
                        class="p-6 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center bg-gray-50/50 dark:bg-gray-700/50">
                        ...
                        <table class="w-full text-left border-collapse">
                            ...
                            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                                @forelse($hotelBookings as $booking)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition">
                                        <td class="px-6 py-4 font-bold text-gray-900 dark:text-white">
                                            #HTL-{{ str_pad($booking->id, 5, '0', STR_PAD_LEFT) }}
                                        </td>
                                        <td class="px-6 py-4">
                                            <div class="font-bold text-gray-900 dark:text-white">
                                                {{ $booking->user->name ?? 'Guest' }}</div>
                                        ...
                                        <td class="px-6 py-4 font-medium text-gray-700 dark:text-gray-300">
                                            {{ Str::limit($booking->hotel->name ?? 'N/A', 30) }}
                                        </td>
                                        <td class="px-6 py-4 font-black text-green-600 dark:text-green-400">
                                            ${{ number_format($booking->total_price, 2) }}
                                        </td>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="px-6 py-8 text-center text-gray-500 font-medium">No hotel
                                            bookings found.</td>
                                    </tr>
                                @endforelse
                            </tbody>
